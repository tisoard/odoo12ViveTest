...
class AttachmentTestCase(common.TransactionCase):
    ...
    def test_archive_attachments(self):
        attachments = self.env['ir.attachment'].sudo().search([('type', '=', 'binary')], limit=2)
        self.assertTrue(attachments.archive())
        self.assertTrue(attachments.archive_with_structure())